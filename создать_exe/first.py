...
WIDTH = 1800
HEIGHT = 1000
MINEDA0 = int(input('минимальное количество еды в первую итерацию(360):') or 350)
MAXEDA0 = int(input('максимальное количество еды в первую итерацию(500):') or 500)
MINEDA1 = int(input('минимальное количество еды после первой итерации(200):') or 200)
MAXEDA1 = int(input('максимальное количество еды после первой итерации(400):') or 400)
FPS = 60
RADB = 0
RADL = 1000
DEAD = 0
...
MUT = int(input('') or 3)
IT = 1
STARTMIN = int(input('минимальный начальный размер бактерии(12):') or 12)
STARTMAX = int(input('максимальный начальный размер бактерии(20):') or 20)
TIC = int(input('') or 10)
CLIVES = 0
LIVE = 0
LIVES = int(input('количество бактерий(50):') or 50)
CDEAD = 0
CLIVE = 0
...
pperpoe
    ...
    def __init__(self, types, *args):
        ...
        if types == 0:
            self.radius = random.randint(STARTMIN, STARTMAX)
        else:
            self.radius = args[0]
        if self.radius < RADL:
            RADL = self.radius
        ...
        if types == 0:
            ...
            if self.eat < self.radius // 2:
                ...
                if 1 + random.random() <= self.eat / (self.radius // 2):
                    a = 0
                    if random.randint(1, MUT) == 1:
                        a = random.randint(round(self.rect[2] / -6), round(self.rect[2] / 6))
...
if __name__ == '__main__':
    ...
    for i in range(LIVES):
        pigs.add(Pig(0))
    FOOD = random.randint(MINEDA0, MAXEDA0)
    for i in range(FOOD):
        foods.add(Food())
    ...
    text2 = f2.render(f'ЦИКЛ № {IT} (ИЗ {TIC})', True, (153, 102, 204))
    ...
    pygame.time.set_timer(pygame.USEREVENT, 20000, TIC)
    pygame.time.set_timer(pygame.QUIT, 20000 * TIC + 1000, 1)
    while running:
        screen.blit(back, back_rect)
    ...
    #                FOOD = random.randint(250 * 0.35 // 1 + DEAD // 2, 500 * 0.7 // 1 + DEAD)):
                    FOOD = random.randint(MINEDA1, MAXEDA1)
                    for i in range(FOOD):
                        foods.add(Food())
                    DEAD = 0
                    CDEAD = 0
