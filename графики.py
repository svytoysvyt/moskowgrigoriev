...
#        x14.append(int(i[9]))
    ...
    if i[0] == 5:
        pass
x3 = sorted(x3.items(), key=lambda x: x[0])
x4 = sorted(x4.items(), key=lambda x: x[0])
x3 = dict(x3)
x4 = dict(x4)
x11 = np.array(x11)
x12 = np.array(x12)
...
#ax1.plot(y1, x14, color='purple', linewidth=2, linestyle='-', label='всего бактерии')
